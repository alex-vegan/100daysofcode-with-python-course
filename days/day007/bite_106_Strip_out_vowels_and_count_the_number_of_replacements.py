...
def strip_vowels(text: str) -> (str, int):
    ...
    start_num_asterisk = Counter(text)['*']
    text_with_asterisk = ''
    for letter in text:
        if not letter.lower() in vowels:
            text_with_asterisk += letter
        else:
            text_with_asterisk += '*'
    end_num_asterisk = Counter(text_with_asterisk)['*']
    num_asterisk = end_num_asterisk - start_num_asterisk
    return (text_with_asterisk, num_asterisk)
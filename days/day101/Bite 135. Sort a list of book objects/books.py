...
def sort_books_by_first_authors_last_name(books=books):
    ...
    Automate the Boring Stuff with Python
    """
    return sorted(books, key=lambda x: x.authors.split()[1].rstrip(','))
...
def sort_books_by_published_date(books=books):
    ...
    Python Interviews
    """
    return sorted(books, key=lambda x: datetime(*map(int, x.published.split('-'))))


if __name__ == "__main__":
    #print(sort_books_by_number_of_page()[-1].title)
    #print(sort_books_by_len_of_title()[-1].title)
    #print(sort_books_by_published_date()[-1].title)
    print(sort_books_by_first_authors_last_name()[-1].title)
